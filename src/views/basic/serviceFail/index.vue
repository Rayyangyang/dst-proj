<template>
  <div>
    <BasicTable :showIndexColumn="false" @register="registerTable" @change="handleTableChange">
          ...
          :actions="[
            {
              ...
              ifShow: () => {
                return hasPermission(ServiceFailManageEnum.ServiceFail_EDIT);
              },
            },
            {
              label: '删除',
              ...
              ifShow: () => {
                return hasPermission(ServiceFailManageEnum.ServiceFail_DELETE);
              },
            },
          ]"
        />
      ...
      <template #tableTitle v-if="hasPermission(ServiceFailManageEnum.ServiceFail_ADD)">
...
<script lang="ts">
  ...
  import { actionServiceFailPage, actionServiceFailDelete } from '/@/api/basic/service';
  import { ServiceFailManageEnum } from '/@/assets/permission/basic';
    ...
    setup() {
      ...
      async function getList() {
        try {
          ...
          setLoading(true);
          const { status, data } = await actionServiceFailPage(paramsVo);
      ...
      const handleDelete = (data: any) => {
        Modal.confirm({
          ...
          onOk() {
            actionServiceFailDelete(data?.id).then((res) => {
              if (res.status) {
